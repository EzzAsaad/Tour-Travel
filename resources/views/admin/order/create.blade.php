                    ...
                    <div class="form-group row">
                        <div class="form-group col-6">
                            ...
                            <select  class="form-control"  id="user_id" name="user_id" required>
                                <option selected disabled value="">{{__('order.assignuser')}}</option>
                    ...
                    <table @if(App::isLocale('ar'))dir="rtl" @endif class="table table-bordered" id="dynamicAddRemoveProduct">
                        ...
                        <tr>
                            ...
                            <td>   <select onchange="return getbroductsize(this,0)" class="form-control" name="products_orders[0][product_id]">
                                    <option selected disabled value="">{{__('order.assignprod')}}</option>
                                @foreach($products as $prod)
                                    <?php
                                        $name = json_decode($prod->name);
                                        if(App::isLocale('ar')){
                                            $name = $name->ar;
                                        }
                                        else{
                                    ...
                                    ?>
                                    <option  value="{{$prod->id}}">{{$name}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
            ...
            $.ajax({
                ...
                success:function(data) {
                    ...
                    for(j = 0 ; j<data.products_sizes.length;j++){
                        let name = JSON.parse(data.products_sizes[j].size)
                        @if(App::isLocale('ar'))
                        name = name.ar;
                        @else
                        name = name.en;
                        @endif
                        $(`#prodsize${val}`).append(`<option value="${data.products_sizes[j].id}">${name}</option>`)
                    }
                }
            });
        }
