...
//                         dd(Auth::user());
                        ...
                        <td class="text-center">
                            @if(session()->has('flag') && session()->get('flag') == '1')
                                <a style="background-color: #D7DDE2;padding: 10px;" class="btn btn-sm btn-alt-secondary" href="{{ route('Admin.ViewEmployee', ["lang"=>'ar',$user->id]) }}" data-toggle="tooltip" title="{{__('users.view')}}">
                                    <i class="fa fa-fw fa-eye"></i>
                                </a>
                                @if($delete==1)
                                    <a style="background-color:#f44336;padding:10px;" class="btn btn-sm btn-light js-tooltip-enabled" href="{{ route('Admin.deleteEmployee', ["lang"=>'ar',$user->id]) }}" data-toggle="tooltip" title="{{__('users.delete')}}">
                                        <i class="fa fa-fw fa-times"></i>
                                    </a>
                                @endif
                                @if($edit==1)
                                    <a class="btn btn-sm btn-alt-danger" href="{{ route('Admin.EditUser',["lang"=>'ar',$user->id]) }}" data-toggle="tooltip" title="{{__('users.edit')}}">
                                        <i class="fa fa-fw fa-pencil-alt"></i>
                                    </a>
                                @endif
                            @else
                                <a class="btn btn-sm btn-alt-secondary" href="{{ route('Admin.ViewUser', ["lang"=>'ar',$user->id]) }}" data-toggle="tooltip" title="{{__('users.view')}}">
                                    <i class="fa fa-fw fa-eye"></i>
                                </a>

                                @if($delete1==1)
                                    <a class="btn btn-sm btn-alt-danger" href="{{ route('Admin.deleteUser', ["lang"=>'ar',$user->id]) }}" data-toggle="tooltip" title="{{__('users.delete')}}">
                                        <i class="fa fa-fw fa-times"></i>
                                    </a>
                                @endif
