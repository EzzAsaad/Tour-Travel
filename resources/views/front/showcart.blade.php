...
@section('content')
    ...
    <form action='{{route('Front.createorder',['lang'=>(App::isLocale('ar'))? 'ar' : 'en'])}}' method='POST'>
        @csrf
                            ...
                            <table class="table text-center table-cart" @if(App::isLocale('ar'))dir="rtl" @endif>
                                ...
                                <tr id="abId0.888403729209045">
                                    <th scope="row" class="text-center">
                                        <?php $tmp_prod = App\Models\Product::where('id',$product->product_id)->first()->image; ?>
                                        <img style="width: 105px;height: 105px" src="{{asset($tmp_prod)}}" alt="" class="img-fluid w-25 mr-3">
                                    ...
                                    <td class="text-right"><a href="{{route('Front.deletecart',['lang'=>(App::isLocale('ar'))? 'ar' : 'en','id'=>$product->id])}}"><i class="ti-close close"></i></a></td>
...
{{--                                <button type="submit" class="@if(App::isLocale('en')) float-right @endif btn" style="background-color: red; color: white">{{__('front.send')}}</button>--}}
                ...
                <div class="row mt-4   @if(App::isLocale('ar')) text-right @else justify-content-end @endif " @if(App::isLocale('ar')) dir="rtl" @endif>
                    <div class="col-6 px-xl-5 py-4 card border-0  ">
                        ...
                        <div class="media align-items-center border-bottom font-weight-medium py-3">
                            ...
                            <span id="totalprods" class="text-dark">237.00<span>L.E</span></span>
                        </div>
                        ...
                        <div class="media align-items-center border-bottom font-weight-medium py-3">
                            ...
                            <span id="totaladdons" class="text-dark" >237.00<span>L.E</span></span>
                        </div>

                        <div class="media justify-content-between align-items-center py-3">
                            <span class="text-dark ">{{__('front.total')}}</span>
                            <span id="totalOrder" class="text-dark font-weight-bold">237.00<span>L.E</span></span>
                        </div>
    ...
    <script>
        ...
        editOrderSummary = ()=>{
            let totalProds = document.getElementById('totalprods');
            let sumProducts = 0;
            @foreach($cart as $prod)
            var prodsArray = document.getElementsByName('prods[{{$prod->id}}][price_prod_hidden]');
            sumProducts += parseInt(prodsArray[0].value);
            @endforeach
                totalProds.innerHTML =`${sumProducts}L.E.`;

            let totalAddons = document.getElementById('totaladdons');
            let sumAddons = 0;
            @foreach($addons as $addon)
            var addonsArray = document.getElementsByName('addon[{{$addon->id}}][subtotal]');
            sumAddons += parseInt(addonsArray[0].value);
            @endforeach
                totalAddons.innerHTML =`${sumAddons}L.E.`;

            let totalOrder = document.getElementById('totalOrder');

            totalOrder.innerHTML =`${sumProducts+sumAddons}L.E.`;
        }
    </script>
@endsection
